...
#define TOY_DIST_OFFSET 0.20   //in meters: used to offset toy location before sending coordinates to navigation so that the robot stops before it.
#define TIME_COUNT 6        //
...
//   ros::ServiceClient initGripperClient = n.serviceClient<std_srvs::Empty>("init_gripper");
    ...
    basketLocation[0].x = 11.815;  
    basketLocation[0].y = 11.834;
    basketLocation[0].s = -100;
    //To DO: Define type2 (blue) basket location
    basketLocation[1].x = 14.459;
    basketLocation[1].y = 12.272;
    basketLocation[1].s = 20;
...
//      scanf("%d", &nextState);    //testing
        ...
        case ID:
            ...
            vantageIndex = toyType-1;   // the next vantage point to go to is the one neares to the basket
            state = PICKUP;
            break;
        case PICKUP:
