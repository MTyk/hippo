...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub = nh.subscribe("/pose_error", 1000, getError);
    ...
    while (ros::ok()){
        ...
        if ( (angError<0 && angError<angErrorTolerance) || (angError>0 && angError>angErrorTolerance) ){ //abs(err)>tolerance?
            ...
            cmd.angular.x = pAction + dAction + iAction;
            angErrorLast = angError;
            
            //Set linear velocity to 0 to ensure only orientation is controlled
            cmd.linear.x = 0;
        } else if ( (linError<0 && linError<linErrorTolerance) || (linError>0 && linError>linErrorTolerance) ){
